...
public class NotificationServiceTestSuite {
    ...
    public void subscribedClientShouldReceiveNotification() {
        NotificationService notificationService = new NotificationService(); //tworzymy obiekt
        Client client = Mockito.mock(Client.class); //tworzymy klienta
        notificationService.addSubscriber(client); //dodajemy klienta
        Notification notification = Mockito.mock(Notification.class);

        notificationService.sendNotification(notification); //wysyłamy notifikację
        Mockito.verify(client, Mockito.times(1)).receive(notification); //powinien ptrzymać 1 powiadomienie
